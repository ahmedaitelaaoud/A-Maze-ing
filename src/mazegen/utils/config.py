...
def validate_config(raw_settings: Dict[str, str]) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ValueError("Error: WIDTH and HEIGHT must be valid integers.")

    w, h = clean_settings["WIDTH"], clean_settings["HEIGHT"]

    if w < 3 or h < 3:
        raise ValueError("Error: WIDTH and HEIGHT must be at least 3.")
    ...
    raw_algo = raw_settings.get("ALGO", "DFS").strip().upper()

    if raw_algo not in valid_algos:
        raise ValueError(f"Error: Unsupported ALGO '{raw_algo}'. Supported: {valid_algos}")

    clean_settings["ALGO"] = raw_algo
